...
# =========================
# 4) Save final structure (+ magmoms) from vasprun.xml
# =========================
final_atoms = atoms

if os.path.isfile("vasprun.xml"):
    try:
        # Read FINAL ionic step (important!)
        final_atoms = read("vasprun.xml", index=-1)

        # Try to extract per-atom magnetic moments and store them explicitly
        try:
            magmoms = final_atoms.get_magnetic_moments()
            if magmoms is not None and len(magmoms) == len(final_atoms):
                magmoms = np.array(magmoms, dtype=float)
                final_atoms.set_initial_magnetic_moments(magmoms)
                final_atoms.arrays["final_magmoms"] = magmoms
                print("Saved magmoms to initial_magmoms and arrays['final_magmoms']")
        except Exception:
            print("WARNING: Could not read magmoms from vasprun.xml (continuing).")

    except Exception:
        final_atoms = atoms
        print("WARNING: failed to read vasprun.xml, using current atoms object.")

write("final_with_calculator.traj", final_atoms)
write("final_with_calculator.cif", final_atoms)

# Also write json directly from final_atoms (keeps arrays like magmoms best)
#write("final_with_calculator.json", final_atoms)
